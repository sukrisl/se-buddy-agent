...
| 3 | Venv is created at `vendor/.venv`, capellambse installed **from the vendored submodule**, transitive deps from the pinned lockfile | Sec.5.1 | [bin/_bootstrap.py](bin/_bootstrap.py) `build_venv()`, `_install_capellambse()` | **Partly superseded by AC-0008 — a deliberate, recorded departure from Sec.5.1, not a regression.** Transitive deps still come from the lockfile and never an index resolution. capellambse itself is now installed from a prebuilt wheel at `pyproject.toml`'s pinned version, with the vendored submodule as the fallback where no wheel matches the platform. The reason: building the submodule requires a Rust toolchain on every install, capellambse 0.8.1 publishes wheels for every platform this targets, and on the first real install the compiler prerequisite is where setup stopped. What is given up is stated rather than hidden — a version pin is not a commit pin, so the artefact now comes from an index rather than a SHA this repo records. `doctor` still refuses on drift from the pin (row 9), and hash-pinning the wheel in `lockfile` would recover the rest; it is not done |
...
| 16 | `project-init` scaffolds `profile.yaml`/`domain.md`/`viewpoints.yaml`/`principles.yaml`, refuses to overwrite, and `doctor` reports what's still missing as `SUPPLY` asks | Sec.5.3 | [skills/project-init/SKILL.md](skills/project-init/SKILL.md), [src/se_buddy/scaffold.py](src/se_buddy/scaffold.py), [src/se_buddy/profile.py](src/se_buddy/profile.py) | **Enforced**, and now fully closable. Scaffolding and the completeness check were real from Phase 1 (scaffold → `doctor` → correct `SUPPLY` ask count); `write memory viewpoints`/`write memory principles` (added in Phase 2's second pass) now let the engineer actually complete a viewpoint/principle, verified live end-to-end: writing a real viewpoint made its `SUPPLY` ask auto-resolve on the next `se-buddy asks` run. **Superseded by AC-0008**: `profile.yaml`'s fields and `domain.md`'s six sections were previously described here as "permanently hand-edited by design". That was a skill-level decision, not a spec constraint, and the first real install showed what it cost — a domain pack carrying real content under the template's own unreplaced heading, reported as complete. Both now have gated verbs (`write-profile`, `write-domain`, `se_buddy.profile_detect`, `se_buddy.domain_pack`): `profile.yaml`'s four fields are detected from the project's own files and confirmed with their provenance shown; `domain.md` is interviewed, drafted and written under the same TTY gate as `write memory`. Neither is a `write memory` *domain* (Sec.7.3's list is unchanged) — they are their own verbs. Completeness now checks the pack's content against Sec.5.4's six sections, refusing on structural gaps and only reporting the placeholder heuristics |
| 16b | A project that installs this gets a thin, generated `CLAUDE.md` of its own, pointing at its profile | Sec.5.2 | [src/se_buddy/project_claude_md.py](src/se_buddy/project_claude_md.py), [src/se_buddy/commands/write_claude_md.py](src/se_buddy/commands/write_claude_md.py) | **Enforced since AC-0008; specified from the start and unbuilt until then** — Sec.5.2's project layout lists it and nothing generated it, so a fully-installed, fully-profiled project still gave a session no automatic pointer to any of it. Numbered `16b` rather than appended because it belongs with row 16's init work. Not TTY-gated, and deliberately: it carries no architectural content and writes only between two markers, so it can neither assert anything the engineer must judge nor lose anything they wrote — the same carve-out Sec.7.3 gives `write propose`. Verified live against a project with a pre-existing hand-written `CLAUDE.md`: appended on first run, replaced in place on the second, the engineer's own content untouched by both |
